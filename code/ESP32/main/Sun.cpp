...
namespace CLASSICDIY
{
	...
	void Sun::calcSun(time_t* unixTime)
	{
		struct tm *ptm = gmtime(unixTime);
		centuries = ptm->tm_year / 100;
		leaps = centuries / 4;
		leapDays = 2 - centuries + leaps;         // note is negative!!
		yearDays = 365.25 * (ptm->tm_year + 4716);     // days until 1 jan this year
		monthDays = 30.6001* (ptm->tm_mon + 1);    // days until 1st month
		JulianResult = leapDays + ptm->tm_mday + monthDays + yearDays - 1524.5;

		fltMins = ((float)ptm->tm_min / 60.0) / 24.0;
		fltHrs = (float)ptm->tm_hour / 24.0;
		...
		double trueSolarTime = ptm->tm_hour * 60 + ptm->tm_min + ptm->tm_sec / 60.0 + solarTimeFix;
